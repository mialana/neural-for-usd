...
function(usd_library NAME)
  ...
  _usd_install_resource_files(${NAME} TYPE SHARED RESOURCE_FILES
                              ${args_RESOURCE_FILES})
...
function(usd_plugin NAME)
  ...
  set(multiValueArgs CPPFILES LIBRARIES INCLUDE_DIRS RESOURCE_FILES)
  ...
  _usd_install_resource_files(${NAME} TYPE "PLUGIN" RESOURCE_FILES
                              ${args_RESOURCE_FILES})
endfunction()

# Public entry point for building python-and-resource-files only library. This
# was _specifically_ exposed to produce plugins for usdview, but can be useful
# for deploying python plugins in general that adhere to the installation
# structure prescribed by USDPluginTools.
#
function(usd_python_library NAME)
  ...
  set(multiValueArgs PYTHON_INSTALL_PREFIX PYTHON_FILES RESOURCE_FILES)
  ...
  if(ENABLE_PYTHON_SUPPORT)
    _usd_python_module(
      ${NAME} PYTHON_INSTALL_PREFIX ${args_PYTHON_INSTALL_PREFIX} PYTHON_FILES
      ${args_PYTHON_FILES})
...
function(usd_executable EXECUTABLE_NAME)
  ...
  set(multiValueArgs CPPFILES LIBRARIES INCLUDE_DIRS)
...
function(usd_test TEST_TARGET)
  ...
  # Define a new executable.
  add_executable(${TEST_TARGET} ${args_CPPFILES})

  # Apply properties.
  _usd_target_properties(${TEST_TARGET} INCLUDE_DIRS ${args_INCLUDE_DIRS}
                         LIBRARIES ${args_LIBRARIES})

  if(CMAKE_CXX_COMPILER_ID STREQUAL "GNU")
    target_link_options(${TEST_TARGET} PRIVATE "-Wl,--no-as-needed")
  endif()
...
# Adds a USD-based python test which is executed by CTest. The python file is
# simply executed by the python interpreter with no special arguments.
function(usd_python_test TEST_TARGET PYTHON_FILE)
  if(NOT ENABLE_PYTHON_SUPPORT)
    message(
      STATUS
        "ENABLE_PYTHON_SUPPORT is OFF, skipping python test: ${TEST_PREFIX} ${PYTHON_FILE}"
    )
    return()
  endif()
...
function(_usd_cpp_library NAME)
  ...
  if(NOT args_TYPE STREQUAL "PLUGIN" AND NOT args_TYPE STREQUAL "SHARED")
    message(FATAL_ERROR "Building library type '${args_TYPE}' not supported.")
  endif()
  ...
  if(args_PUBLIC_HEADERS_INSTALL_PREFIX)
    set(PUBLIC_HEADERS_INSTALL_PREFIX
        ${args_PUBLIC_HEADERS_INSTALL_PREFIX}/${NAME})
  else()
  ...
  file(
    ...
    DESTINATION
      ${CMAKE_BINARY_DIR}/${CMAKE_INSTALL_INCLUDEDIR}/${PUBLIC_HEADERS_INSTALL_PREFIX}
  )
  ...
  # Convert "PLUGIN" into "MODULE". (MODULE is CMake terminology for a dynamic
  # lib only intended to be dlopen'ed at runtime).
  ...
  target_sources(${NAME} PRIVATE ${args_CPPFILES} ${args_PUBLIC_HEADERS})

  # Apply common compiler properties, and include path properties.
  _usd_target_properties(${NAME} INCLUDE_DIRS ${args_INCLUDE_DIRS} LIBRARIES
                         ${args_LIBRARIES})

  _usd_compute_library_install_and_file_prefix(
    ${args_TYPE} LIBRARY_INSTALL_PREFIX LIBRARY_FILE_PREFIX)

  if(args_TYPE STREQUAL "PLUGIN")
    # Install the plugin. We do not need to export the target because plugins
    # are _not_ meant to be built against.
    install(TARGETS ${NAME} LIBRARY DESTINATION ${LIBRARY_INSTALL_PREFIX})
  else()
    # Setup SOVERSION & VERSION properties to create NAMELINK, SONAME, and
    # actual library with full version suffix.
    set_target_properties(
      ${NAME} PROPERTIES SOVERSION ${CMAKE_PROJECT_VERSION_MAJOR}
  ...
  # Mirror installation structure in PROJECT_BINARY_DIR - for running tests
  # against.
  if(BUILD_TESTING)
    add_custom_command(
      TARGET ${NAME}
      POST_BUILD
      COMMAND
        ${CMAKE_COMMAND} -E create_symlink $<TARGET_FILE:${NAME}>
        ${PROJECT_BINARY_DIR}/${LIBRARY_INSTALL_PREFIX}/$<TARGET_FILE_NAME:${NAME}>
    )
  endif()

  # Update the target properties.
  set_target_properties(${NAME} PROPERTIES PREFIX "${LIBRARY_FILE_PREFIX}")
...
function(_usd_python_module NAME)
  ...
  set(multiValueArgs LIBRARIES INCLUDE_DIRS PYTHON_INSTALL_PREFIX PYTHON_FILES
                     PYMODULE_CPPFILES)
  ...
  if(args_PYTHON_INSTALL_PREFIX)
    set(PYTHON_INSTALL_PREFIX
        ${CMAKE_INSTALL_LIBDIR}/${USD_PYTHON_DIR}/${args_PYTHON_INSTALL_PREFIX}/${PYTHON_MODULE_NAME}
    )
  else()
    set(PYTHON_INSTALL_PREFIX
        ${CMAKE_INSTALL_LIBDIR}/${USD_PYTHON_DIR}/${PYTHON_MODULE_NAME})
  endif()
  ...
  if(args_PYMODULE_CPPFILES)
    ...
    add_library(${PYLIB_NAME} MODULE)

    # Add sources for building the target.
    target_sources(${PYLIB_NAME} PRIVATE ${args_PYMODULE_CPPFILES})

    # Lose the library prefix.
    if(MSVC)
      set_target_properties(${PYLIB_NAME} PROPERTIES PREFIX "" SUFFIX ".pyd")
    else()
      set_target_properties(${PYLIB_NAME} PROPERTIES PREFIX "")
    endif()

    # Apply common compilation properties, and include path properties.
    _usd_target_properties(
      ${PYLIB_NAME}
      INCLUDE_DIRS
      ${args_INCLUDE_DIRS}
      DEFINES
    ...
    if(BUILD_TESTING)
      add_custom_command(
        ...
        POST_BUILD
        COMMAND
          ${CMAKE_COMMAND} -E create_symlink $<TARGET_FILE:${PYLIB_NAME}>
          ${PROJECT_BINARY_DIR}/${PYTHON_INSTALL_PREFIX}/$<TARGET_FILE_NAME:${PYLIB_NAME}>
      )
    endif()

    # Install python module library.
    install(TARGETS ${PYLIB_NAME} RENAME
  ...
  if(args_PYTHON_FILES)
    ...
    if(BUILD_TESTING)
      foreach(pythonFile ${args_PYTHON_FILES})
        file(COPY ${CMAKE_CURRENT_SOURCE_DIR}/${pythonFile}
             DESTINATION ${PROJECT_BINARY_DIR}/${PYTHON_INSTALL_PREFIX})
      endforeach()
    endif()

    # Install python files.
    install(FILES ${args_PYTHON_FILES} DESTINATION ${PYTHON_INSTALL_PREFIX})
  endif()
...
function(_usd_install_resource_files NAME)
  ...
  # Compose the full name of the library. This will be used when performing
  # variable substition on the plugInfo.json resource file.
  ...
  set(RESOURCES_INSTALL_PREFIX
      ${LIBRARY_INSTALL_PREFIX}/${USD_PLUG_INFO_ROOT_DIR})
  ...
  foreach(resourceFile ${args_RESOURCE_FILES})
    ...
    if(${resourceFile} STREQUAL "plugInfo.json")
      file(
        RELATIVE_PATH RESOURCE_TO_LIBRARY_PATH
    ...
    if(BUILD_TESTING)
      file(
        COPY ${resourceFile}
        DESTINATION
          ${PROJECT_BINARY_DIR}/${RESOURCES_INSTALL_PREFIX}/${NAME}/${USD_PLUG_INFO_RESOURCES_DIR}
      )
    endif()
...
#
# Outputs: LIBRARY_INSTALL_PREFIX: The sub-directory under installation root
# where the shared library will be deployed. LIBRARY_FILE_PREFIX: The filename
# prefix of the shared library. ("lib" on Linux)
function(_usd_compute_library_install_and_file_prefix TYPE
         LIBRARY_INSTALL_PREFIX LIBRARY_FILE_PREFIX)
...
# Converts a library name, such as _tf.so to the internal module name given our
# naming conventions, e.g. Tf
function(_usd_get_python_module_name LIBRARY_FILENAME MODULE_NAME)
  # Library names are either something like tf.so for shared libraries or
  # _tf.pyd/_tf_d.pyd for Python module libraries. We want to strip off the
  # leading "_" and the trailing "_d".
...
# Set-up runtime environment variables for the test. When USE_PYTHONPATH is one,
# will include project python libraries
function(_usd_set_test_properties TARGET_NAME USE_PYTHONPATH)
  # The paths refer to the build tree (which mirrors the final installation).
  # The first path for an env var needs the 'VARNAME=' prepended. On windows,
  # calls to "$ENV{}" must be sanitized to replace back slashes with forward
  # slashes Env vars after the first must be prepended with a double escaped
  # semicolon to be recognized.
  ...
  if(MSVC)
    string(REGEX REPLACE "\\\\" "/" TEST_PXR_PLUGINPATH_NAME
                         "${TEST_PXR_PLUGINPATH_NAME}")
  endif()
  ...
  if(MSVC)
    ...
    list(APPEND TEST_PATH "${PROJECT_BINARY_DIR}/${CMAKE_INSTALL_LIBDIR}"
  ...
  set_tests_properties(${TARGET_NAME} PROPERTIES ENVIRONMENT "${TEST_ENV_VARS}")
