...
Scene::Scene()
    ...
    , mWon(false)
{
    mStage = UsdStage::Open("/Users/Dev/Projects/Neural-for-USD/src/usdSimple/board.usda");
    UsdStageRefPtr switchStage = UsdStage::Open(
        "/Users/Dev/Projects/Neural-for-USD/src/usdSimple/switch.usda");
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            ...
            UsdGeomXformCommonAPI(xfPrim).SetTranslate(GfVec3f(105.0f * i, 0.0f, -105.0f * j));

            // Set attributes
            // Number of turns
            auto turnsAttr = xfPrim.CreateAttribute(TfToken("turns"), SdfValueTypeNames->Int, true);
            turnsAttr.Set(rand() % 2);

            // Index
            auto indexIAttr = xfPrim.CreateAttribute(TfToken("indexI"),
                                                     SdfValueTypeNames->Int,
                                                     true);
            indexIAttr.Set(i);
            auto indexJAttr = xfPrim.CreateAttribute(TfToken("indexJ"),
                                                     SdfValueTypeNames->Int,
                                                     true);
            indexJAttr.Set(j);
            ...
            instPrim.GetReferences().AddReference(switchStage->GetRootLayer()->GetIdentifier(),
                                                  SdfPath("/switch1"));
        }
    }
...
void Scene::prepare(float seconds)
{
    ...
    for (const auto& prim : mBoard.GetChildren()) {
        ...
        xf.GetXformVectors(&translation,
                           &rotation,
                           &scale,
                           &pivot,
                           &rotOrder,
...
void Scene::draw(int width, int height)
{
    ...
    light.SetPosition(GfVec4f(static_cast<float>(position[0]),
    ...
    GlfSimpleLightingContextPtr p = GlfSimpleLightingContextPtr();
    mRenderer.SetLightingState(lights, material, sceneAmbient);
    mRenderer.SetCameraState(viewMat, projMat);
...
void Scene::cursor(float x, float y)
{
    ...
    auto frustum = cameraFrustum.ComputeNarrowedFrustum(GfVec2d(2.0 * x - 1.0,
                                                                2.0 * (1.0 - y) - 1.0),
                                                        size);
    ...
    if (mRenderer.TestIntersection(frustum.ComputeViewMatrix(),
