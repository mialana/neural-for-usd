...
MyFrameRecorder::MyFrameRecorder(const TfToken& rendererPluginId, bool gpuEnabled)
...
void MyFrameRecorder::SetActiveRenderSettingsPrimPath(SdfPath const& path)
{
...
void MyFrameRecorder::SetActiveRenderPassPrimPath(SdfPath const& path)
{
...
void MyFrameRecorder::SetColorCorrectionMode(const TfToken& colorCorrectionMode)
{
...
void MyFrameRecorder::SetCameraLightEnabled(bool cameraLightEnabled)
{
    _cameraLightEnabled = cameraLightEnabled;
}

void MyFrameRecorder::SetDomeLightVisibility(bool domeLightsVisible)
{
    _domeLightsVisible = domeLightsVisible;
}

void MyFrameRecorder::SetIncludedPurposes(const TfTokenVector& purposes)
{
...
{
public:
    TextureBufferWriter(UsdImagingGLEngine* engine)
        : _engine(engine)
...
private:
    ...
    void* _Map()
    {
        if (_colorTextureHandle) {
            size_t size = 0;
            _mappedColorTextureBuffer = HdStTextureUtils::HgiTextureReadback(_engine->GetHgi(),
                                                                             _colorTextureHandle,
                                                                             &size);
...
bool MyFrameRecorder::Record(const UsdStagePtr& stage,
                             ...
                             const std::string& outputImagePath)
{
    ...
    const size_t imageHeight = std::max<size_t>(static_cast<size_t>(static_cast<float>(_imageWidth)
                                                                    / aspectRatio),
                                                1u);
    ...
    if (usdCamera) {
        ...
        _imagingEngine.SetCameraState(frustum.ComputeViewMatrix(),
    ...
    _imagingEngine.SetRendererSetting(HdRenderSettingsTokens->domeLightCameraVisibility,
...
uint64_t MyFrameRecorder::getTextureHandle(const UsdStagePtr& stage,
                                           const UsdGeomCamera& usdCamera,
                                           ...
                                           double devicePixelRatio)
{
    ...
    VtValue aov;
    HgiTextureHandle aovTexture = _imagingEngine.GetAovTexture(HdAovTokens->color);

    _interop.TransferToApp(_hgi,
                           aovTexture,
                           /*srcDepth*/ HgiTextureHandle(),
                           HgiTokens->OpenGL,
                           VtValue(frameBufferId),
                           GfVec4i(0, 0, _imageWidth * devicePixelRatio, imageHeight * devicePixelRatio));

    return writer._colorTextureHandle.GetId();
}
