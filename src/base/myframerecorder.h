...
public:
    ...
    /// The \p gpuEnabled argument determines if the MyFrameRecorder
    /// instance will allow Hydra to use the GPU to produce images.
    USDAPPUTILS_API
    MyFrameRecorder(const TfToken& rendererPluginId = TfToken(), bool gpuEnabled = true);
    ...
    uint64_t getTextureHandle(const UsdStagePtr& stage,
                              const UsdGeomCamera& usdCamera,
